...
#define MT 1 
	...
	std::for_each(std::execution::par, m_VerticalIter.begin(), m_VerticalIter.end(), [this](uint32_t y) {
		std::for_each(std::execution::par, m_HorizotalIter.begin(), m_HorizotalIter.end(), [this, y](uint32_t x) {
			...
			accumulattedColor = glm::clamp(accumulattedColor, glm::vec4(0.0f), glm::vec4(1.0f));
			m_ImageData[color_index] = Utils::ConvertToRGBA(accumulattedColor);
		});
	});
#else
...
glm::vec4 Renderer::PerPixel(uint32_t x, uint32_t y) {
	...
	for (int i = 0; i < bounces; i++) {
        ...
        // color mapColor = m_EarthTexture->value(payload.u, payload.v, worldPosition );
        // light.x += (float)mapColor[0];
        // light.y += (float)mapColor[1];
        // light.z += (float)mapColor[2];
